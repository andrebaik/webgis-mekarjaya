...
export interface ScrollExpandProps {
  ...
  /**
   * Tinggi panggung sebagai persen tinggi viewport. Bawaan 100 (satu layar penuh).
   * Turunkan agar medianya tidak pernah memenuhi layar — di dalam halaman yang
   * padat konten, full-bleed membuat pengunjung kehilangan konteks sekitarnya.
   */
  stageVh?: number;
  enabled?: boolean;
  children?: ReactNode;
...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  useEffect(() => {
    ...
    const measure = () => {
      const c = propsRef.current;
      // stageVh membatasi tinggi panggung terhadap viewport. Aslinya baris ini
      // memakai window.innerHeight apa adanya, sehingga mode window-scroll SELALU
      // menghasilkan media setinggi layar penuh tanpa cara membatasinya.
      stageH = c.useWindowScroll
        ? window.innerHeight * (clamp(c.stageVh, 10, 100) / 100)
        : root.clientHeight;
      if (stageH <= 0) return;
